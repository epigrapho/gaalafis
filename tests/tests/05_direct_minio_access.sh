...
expect_stdout_to_contain '{"transfer":"basic","objects":\[{"oid":"test-upload.txt","size":15,"actions":{"upload":{"href":"http://bucket:9000/bucket/testing/objects/test-upload.txt\?X\-Amz\-Algorithm=AWS4\-HMAC\-SHA256&X\-Amz\-Credential=.*&X\-Amz\-Date=[0-9]{8}T[0-9]{6}Z&X\-Amz\-Expires=3600&X\-Amz\-SignedHeaders=host&X\-Amz\-Signature=[a-f0-9]*","expires_in":3600}}}\],"hash_algo":"sha256"}'

# upload file
echo "Test of upload." > /tmp/test-upload.txt
href=$(echo "$stdout_var" | jq -r '.objects[0].actions.upload.href')
run_with_header_capturing_outputs "curl -X PUT '$href' -v -H 'Content-Type: text/plain' -H 'Content-Length: 15' --data-binary @/tmp/test-upload.txt"
