...
# 1) successful access to batch endpoint
...
# 3) upload token used to download missing file (same as download)
...
# 4) download token used to upload file (unauthorized)
