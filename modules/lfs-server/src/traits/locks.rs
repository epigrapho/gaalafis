...
impl Display for LocksProviderError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            LocksProviderError::RequestPreparationFailure(err) => {
                write!(f, "RequestPreparationFailure: {}", err)
            }
            LocksProviderError::RequestExecutionFailure(err) => {
                write!(f, "RequestExecutionFailure: {}", err)
            }
            LocksProviderError::ParsingResponseDataFailure(err) => {
                write!(f, "ParsingResponseDataFailure: {}", err)
            }
